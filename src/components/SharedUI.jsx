...
export function Track({ values, onChange, color = G.gold, size = 16 }) {
  return (
    <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 4 }}>
      {values.map((v, i) => (
        <span
          key={i}
          onClick={() => onChange(values.map((x, j) => (j === i ? !x : x)))}
          style={{
...
export function DamageTrack({ values, onChange, shape = 'square', color = G.gold }) {
  const S = 16;
  const timers = useRef({});
  ...
  return (
    <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: 4 }}>
      {values.map((v, i) => {
        ...
        return (
          <svg
            key={i} width={S} height={S} viewBox={`0 0 ${S} ${S}`}
...
export function Dots({ max = 5, value, onChange, color = G.gold }) {
  return (
...
export function Divider({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '10px 0 8px' }}>
      ...
      {children && (
        <>
          <span style={{ fontFamily: 'Cinzel,serif', fontSize: 11, letterSpacing: '.22em', color: G.gold, fontWeight: 700, whiteSpace: 'nowrap' }}>
            {children}
          </span>
          <div style={{ flex: 1, height: 1, background: G.goldFaint }} />
        </>
...
export function Field({ label, value, onChange, flex = '1 1 160px', fs = 13 }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 5, flex }}>
      <span style={{ fontFamily: 'Cinzel,serif', fontSize: 10, color: G.goldDim, whiteSpace: 'nowrap', fontWeight: 600 }}>
        {label}:
      </span>
      <input
        value={value}
    ...
    <>
      {values.map((v, i) => (
        <input
          key={i}
          value={v}
          onChange={(e) => onChange(values.map((l, j) => (j === i ? e.target.value : l)))}
...
export function TArea({ label, value, onChange, rows = 4 }) {
  return (
    <div style={{ marginBottom: 12 }}>
      {label && (
        <div style={{ fontFamily: 'Cinzel,serif', fontSize: 9, letterSpacing: '.2em', color: G.goldDim, marginBottom: 3 }}>
          {label.toUpperCase()}
        </div>
      )}
      <textarea
...
// ── Sphere editor block (name + dots + 5 description lines)
export function SphereBlock({ sphere, onUpdate }) {
  return (
    <div style={{ marginBottom: 10, padding: '8px 10px', border: `1px solid ${G.goldFaint}`, borderRadius: 3 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <input
          value={sphere.name}
          onChange={(e) => onUpdate({ ...sphere, name: e.target.value })}
          placeholder="Sphere…"
      ...
      {sphere.descs.map((d, i) => (
        <div key={i} style={{ display: 'flex', gap: 5, alignItems: 'baseline', marginBottom: 3 }}>
          ...
          <input
            value={d}
            onChange={(e) => onUpdate({ ...sphere, descs: sphere.descs.map((x, j) => (j === i ? e.target.value : x)) })}
...
export function Toast({ msg }) {
  if (!msg) return null;
  return (
    <div style={{ position: 'fixed', bottom: 80, left: '50%', transform: 'translateX(-50%)', background: '#1a1a0e', border: `1px solid ${G.gold}`, borderRadius: 4, padding: '10px 20px', fontFamily: 'Cinzel,serif', fontSize: 11, letterSpacing: '.15em', color: G.gold, boxShadow: '0 4px 20px #00000088', zIndex: 999, maxWidth: '90vw', textAlign: 'center', whiteSpace: 'nowrap' }}>
      {msg}
    </div>
  );
}
